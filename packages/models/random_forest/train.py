...
def main():
    ...
    param_list = HYPERPARAMETERS["random_forest"][args.mode]
    params = param_list[args.idx] if (args.mode == "tuned" and args.idx < len(param_list)) else param_list[0]
    
    # EXACT FOLDER FOR RANDOM FOREST
    approach_dir = "baseline" if args.mode == "baseline" else f"tuned_idx_{args.idx}"
    RUN_OUTPUT_DIR = os.path.join(PROJECT_ROOT, "experiments", "models", "random_forest", approach_dir, f"exp_{args.exp}")
    ...
    print(f"[*] Loading data for Random Forest - Experiment {args.exp}...")
    ...
    run_name = f"RandomForest_{args.mode}_exp{args.exp}_idx{args.idx}"
    ...
    with mlflow.start_run(run_name=run_name):
        ...
        generate_run_plots(calibrated_model, X_test.values, y_test.values, y_pred, y_proba, feature_names, PLOTS_DIR, "Random Forest")
