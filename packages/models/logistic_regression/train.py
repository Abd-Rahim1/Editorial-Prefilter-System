...
def main():
    ...
    param_list = HYPERPARAMETERS["logistic_regression"][args.mode]
    params = param_list[args.idx] if (args.mode == "tuned" and args.idx < len(param_list)) else param_list[0]
    
    # 2. Output directory mapping (storage/experiments/logistic_regression/...)
    approach_dir = "baseline" if args.mode == "baseline" else f"tuned_idx_{args.idx}"
    RUN_OUTPUT_DIR = os.path.join(PROJECT_ROOT, "experiments", "models", "logistic_regression", approach_dir, f"exp_{args.exp}")
    ...
    print(f"[*] Loading data for Logistic Regression - Experiment {args.exp}...")
    ...
    run_name = f"LogReg_{args.mode}_exp{args.exp}_idx{args.idx}"
    ...
    with mlflow.start_run(run_name=run_name):
        ...
        generate_run_plots(calibrated_model, X_test.values, y_test.values, y_pred, y_proba, feature_names, PLOTS_DIR, "Logistic Regression")
